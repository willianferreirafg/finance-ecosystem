...
export const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setCategoryId('');
      onClose();
      onTransactionCreated(); // Atualiza os dados na tela principal
    } catch (error) {
      console.error('Erro ao salvar transação:', error);
      alert('Falha ao salvar a movimentação. Verifique os dados.');
    } finally {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            <label htmlFor="paid" className="text-sm font-medium text-zinc-300 cursor-pointer select-none">
              {type === 'INCOME' ? 'Recebido' : 'Pago'} (Lançamento compensado)
            </label>
          </div>
